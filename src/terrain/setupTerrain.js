...
const createTerrain = (scene, ridgeHeights) => {
...
export default (scene) => {
  ...
  let ridgedDone = false;

  return {
    onTouchStart: (event) => {
      if (!ridgedDone) {
        drawRidge(scene, ridgeHeights, event.point)
      }
    },
    onTouchMove: (event) => {
      if (!ridgedDone) {
    ...
    onTouchEnd: (event) => {
      if (!ridgedDone) {
        drawRidge(scene, ridgeHeights, event.point, true);
        ridgedDone = true;
        createTerrain(scene, ridgeHeights);
        scene.remove(scene.getObjectByName('ridge'));
      }
    },
  }
};